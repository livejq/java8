...
public class ComparatorTest {
    ...
    public void test1(){
        ...
        // 被排序的集合
        List<User> userList = Lists.newArrayList(new User("Jack",11),new User("livejq",10));
...
//      userList.sort(User::compareByAgeThenName);
        ...
        // 8. Java8,反转排序
        Comparator<User> comparator = (user1, user2) -> user1.getAge().compareTo(user2.getAge());
        userList.sort(comparator);// 先按age排序
        userList.sort(comparator.reversed());// 反转排序
        Assert.assertEquals(userList.get(0).getName(), "Jack");*/

        for(User u : userList) {
            System.out.println("排序后：" + u.getAge());
        }
